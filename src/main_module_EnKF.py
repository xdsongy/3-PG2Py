...
# Purpose:     The main entrance of the Py3PG2 for the dual state ensemble Kalman filter
#
#
...
class run3PG_2_class():
    ...
    def Forcing_Parameter_State_Observed_ValuesCollector(self, forcings, parameters, stateVariables, observedStateVariables):
        """
        Change selected parameters' values. The initial parameters' values could ref Line 1073
        in getSingleSiteParameters.py. The state variables could ref updateOutputData() & writeOutput() in
        main_module_EnKF.py
        """
        pass
    ...
    def run3PG2_EnsembleVersion(self, monthIndex, forcingFactors_enkf, forcingDataPerturbations):
        # increase this counter
        self.monthIncrease += 1
        ...
        # Add disturbances to the selected climate forcing factors as defined in forcingFactors_enkf[] in main.py
        # forcingFactors_enkf = ['mTx', 'mTn', 'mTav', 'mVPD', 'mRain', 'mRad']
        for variable in forcingFactors_enkf:
            # str = 'self.site.' + variable + '+=' + 'forcingDataPerturbations[\'' + variable + '\']'
            # variation = np.random.normal(0, abs(deliverModelRunning[variable]) ** 0.5)
            variation = np.random.normal(0, forcingDataPerturbations[variable] ** 0.5)
            str = 'self.site.' + variable + '+=' + 'variation'
            exec(str)
