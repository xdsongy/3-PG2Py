#-------------------------------------------------------------------------------
# Name:        main_module
# Purpose:     The main entrance of the Py3PG2 
#
#
...
class run3PG_2_class():
    ...
    def __init__(self, singleSiteInstance, bWriteOutput=True):
        ...
        #t0 = time.time() 
        self.run3PG_2()
        #print "...execution time: %4.3f s" % (time.time()-t0)
    ...
    def run3PG_2(self):
        ...
        dbWriteFlag = 0    # 0 = create table; 1 = insert rows of record
        if self.bWriteOutput:
            self.writeOutput(dbWriteFlag)
        ...
        while self.site.standAge <= self.site.endAge - 0.01:    
            ...
            # Write updated output data into sqlite DB (monthly step)
            dbWriteFlag = 1
            if self.bWriteOutput:
                self.writeOutput(dbWriteFlag)
            ...
            #self.site.monthOfYear  = self.fnNextValue(self.site.monthOfYear, 12)
            ...
            if self.site.monthCounter < 12:
                ...
                self.site.monthCounter = 1
            #self.site.monthCounter = self.fnNextValue(self.site.monthCounter, 12)
            
            #===================================================================
            # You can add print clause here to print any output variable you want!
            #===================================================================
            #print self.site.Height            
        
        # Write the last simulation record (last month)
        if self.bWriteOutput:
            self.writeOutput(dbWriteFlag)
        ...
        # end-of-rotation outputs - might be different to end of year output
        self.write3PGresults(self.GlobalVar.opEndRun, self.site.monthCounter)
        
        # Print message when the simulation of this site ended
#        print "Site: %-15s ......simulation ended." % self.site.siteName                          
    ...
    def writeOutput(self, dbWriteFlag):
        ...
        tableName = self.site.siteName.replace(' ', '_')     
